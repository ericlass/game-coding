    ...
    public void SetDisplayMode(DisplayMode mode)
    {
      ...
      if (EnumDisplaySettings(null, -1, ref dm))
      {
        ...
        if (ret == DISP_CHANGE_BADMODE)
          throw new OkuException("The display mode \"" + mode + "\" is not supported! Only use modes returned by Display.GetSupportedModes().");
        else if (ret == DISP_CHANGE_RESTART)
          throw new OkuException("Setting the display mode to \"" + mode + "\" requires a restart!");
        else if (ret != 0)
          throw new OkuException("Error while setting display mode! Windows error code: " + ret + ".");
        ...
        throw new OkuException("Error while setting display mode!");
    }
