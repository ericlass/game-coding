...
namespace OkuBase.GUI
{
  ...
  public class Canvas
  {
    ...
    public void FillRect(Vector2f min, Vector2f max, Color color)
    {
      ...
      OkuManager.Instance.Graphics.DrawMesh(_vertices.InternalArray, null, _colors.InternalArray, 4, PrimitiveType.Quads, null);
    }
    ...
    public void DrawMesh(Vector2f[] points, Vector2f[] texCoords, Color[] colors, int count, PrimitiveType mode, ImageBase texture)
    {
      _vertices.Clear();
      ...
      OkuManager.Instance.Graphics.DrawMesh(_vertices.InternalArray, texCoords, colors, count, mode, texture);
    }
    ...
    {
      DrawMesh(mesh.Vertices.Positions, mesh.Vertices.TexCoords, mesh.Vertices.Colors, mesh.Vertices.Positions.Length, mesh.PrimitiveType, mesh.Texture);
    }
    ...
    public void DrawImage(ImageBase image, Vector2f position)
    {
      position += _area.Min;
      OkuManager.Instance.Graphics.DrawImage(image, position.X, position.Y);
    }

    public void DrawImage(ImageBase image, Vector2f position, Vector2f scale)
    {
      position += _area.Min;
      OkuManager.Instance.Graphics.DrawImage(image, position.X, position.Y, 0, scale.X, scale.Y, Color.White);
    }
    ...
    public void DrawLine(Vector2f start, Vector2f end, float width, Color color)
    {
      ...
      OkuManager.Instance.Graphics.DrawLine(start.X, start.Y, end.X, end.Y, width, color);
    }
    ...
    public void DrawLines(Vector2f[] vertices, Color color, int count, float width, LineMode interpretation)
    {
      ...
      OkuManager.Instance.Graphics.DrawLines(_vertices.InternalArray, color, count, width, interpretation);
    }
    ...
    {
      p += _area.Min;
      OkuManager.Instance.Graphics.DrawPoint(p.X, p.Y, size, color);
    }
    ...
    public void DrawPoints(Vector2f[] points, Color color, int count, float size)
    {
      ...
      OkuManager.Instance.Graphics.DrawPoints(_vertices.InternalArray, _colors.InternalArray, count, size);
    }
