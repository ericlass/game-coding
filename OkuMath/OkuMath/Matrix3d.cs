...
namespace OkuMath
{
  ...
  public struct Matrix3d
  {
    ...
    private static Matrix3d _identity = GetIdentity();
    
    /// <summary>
    /// Helper method to get an identity matrix.
    /// </summary>
    /// <returns>An identity matrix.</returns>
    private static Matrix3d GetIdentity()
    {
    ...
    /// <summary>
    /// Transforms the given array of vectors.
    /// </summary>
    /// <param name="vecs">The vector array to be transformed.</param>
    public void Transform(Vector2f[] vecs)
    {
      for (int i = 0; i < vecs.Length; i++)
        vecs[i] = Transform(vecs[i]);
    }

    /// <summary>
    /// Transforms the given vector array. The result of the transform will be returned in the given target array.
    /// </summary>
    /// <param name="vecs">The vector array that is transformed.</param>
    /// <param name="target">The resulting transformed polygon.</param>    
    public void Transform(Vector2f[] vecs, Vector2f[] target)
    {
      for (int i = 0; i < vecs.Length; i++)
        target[i] = Transform(vecs[i]);
    }
    ...
    public static Matrix3d CreateTranslateScale(double tx, double ty, double sx, double sy)
    {
      Matrix3d result = _identity;
    ...
    /// <param name="angle">The angle to rotate in degrees.</param>
    /// <returns>A new matrix that transforms according to the parameters.</returns>
    public static Matrix3d CreateTranslateRotate(double tx, double ty, double angle)
    {
      Matrix3d result = _identity;
      result.Translate(tx, ty);
      result.Rotate(angle);
    ...
    /// <param name="angle">The angle to rotate in degrees.</param>
    /// <returns>A new matrix that transforms according to the parameters.</returns>
    public static Matrix3d CreateTranslateScaleRotate(double tx, double ty, double sx, double sy, double angle)
    {
      Matrix3d result = _identity;
      result.Translate(tx, ty);
      result.Scale(sx, sy);
