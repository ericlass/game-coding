...
namespace OkuBase.Audio
{
  ...
  /// </summary>
  internal static class WaveLoader
  {
    /// <summary>
    ...
    public static Sound LoadWave(string filename)
    {
    ...
    /// <returns>The loaded wave form or null if an error occurred.</returns>
    public static Sound LoadWave(Stream stream)
    {
    ...
    /// <returns></returns>
    private static bool ReadFormatChunk(BinaryReader reader, Sound wave)
    {
      reader.BaseStream.Seek(0, SeekOrigin.Begin);
    ...
    private static bool ReadChannelData(BinaryReader reader, Sound wave)
    {
      reader.BaseStream.Seek(0, SeekOrigin.Begin);
