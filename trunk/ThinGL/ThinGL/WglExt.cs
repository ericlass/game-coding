...
namespace ThinGL
{
  public class WglExt
  {
    ...
    private delegate wglAllocateMemoryNV (GLsizei size, GLfloat readfreq, GLfloat writefreq, GLfloat priority)
    private delegate wglAssociateImageBufferEventsI3D (IntPtr IntPtr, const HANDLE *pEvent, const LPVOID *pAddress, const DWORD *pSize, UINT count)
    private delegate wglBeginFrameTrackingI3D (void)
    private delegate wglBindDisplayColorTableEXT (GLushort id)
    private delegate wglBindSwapBarrierNV (GLuint group, GLuint barrier)
    private delegate wglBindTexImageARB (IntPtr hPbuffer, int iBuffer)
    private delegate wglBindVideoCaptureDeviceNV (UINT uVideoSlot, IntPtr hDevice)
    private delegate wglBindVideoDeviceNV (IntPtr IntPtr, unsigned int uVideoSlot, IntPtr hVideoDevice, const int *piAttribList)
    private delegate wglBindVideoImageNV (IntPtr hVideoDevice, IntPtr hPbuffer, int iVideoBuffer)
    private delegate wglBlitContextFramebufferAMD (IntPtr dstCtx, GLint srcX0, GLint srcY0, GLint srcX1, GLint srcY1, GLint dstX0, GLint dstY0, GLint dstX1, GLint dstY1, GLbitfield mask, GLenum filter)
    private delegate wglChoosePixelFormatARB (IntPtr IntPtr, const int *piAttribIList, const FLOAT *pfAttribFList, UINT nMaxFormats, int *piFormats, UINT *nNumFormats)
    private delegate wglChoosePixelFormatEXT (IntPtr IntPtr, const int *piAttribIList, const FLOAT *pfAttribFList, UINT nMaxFormats, int *piFormats, UINT *nNumFormats)
    private delegate wglCopyImageSubDataNV (IntPtr hSrcRC, GLuint srcName, GLenum srcTarget, GLint srcLevel, GLint srcX, GLint srcY, GLint srcZ, IntPtr hDstRC, GLuint dstName, GLenum dstTarget, GLint dstLevel, GLint dstX, GLint dstY, GLint dstZ, GLsizei width, GLsizei height, GLsizei depth)
    private delegate wglCreateAffinityDCNV (const IntPtr *phGpuList)
    private delegate wglCreateAssociatedContextAMD (UINT id)
    private delegate wglCreateAssociatedContextAttribsAMD (UINT id, IntPtr hShareContext, const int *attribList)
    private delegate wglCreateBufferRegionARB (IntPtr IntPtr, int iLayerPlane, UINT uType)
    private delegate wglCreateContextAttribsARB (IntPtr IntPtr, IntPtr hShareContext, const int *attribList)
    private delegate wglCreateDisplayColorTableEXT (GLushort id)
    private delegate wglCreateImageBufferI3D (IntPtr IntPtr, DWORD dwSize, UINT uFlags)
    private delegate wglCreatePbufferARB (IntPtr IntPtr, int iPixelFormat, int iWidth, int iHeight, const int *piAttribList)
    private delegate wglCreatePbufferEXT (IntPtr IntPtr, int iPixelFormat, int iWidth, int iHeight, const int *piAttribList)
    private delegate wglDelayBeforeSwapNV (IntPtr IntPtr, GLfloat seconds)
    private delegate wglDeleteAssociatedContextAMD (IntPtr IntPtr)
    private delegate wglDeleteBufferRegionARB (HANDLE hRegion)
    private delegate wglDeleteDCNV (IntPtr IntPtr)
    private delegate wglDestroyDisplayColorTableEXT (GLushort id)
    private delegate wglDestroyImageBufferI3D (IntPtr IntPtr, LPVOID pAddress)
    private delegate wglDestroyPbufferARB (IntPtr hPbuffer)
    private delegate wglDestroyPbufferEXT (IntPtr hPbuffer)
    private delegate wglDisableFrameLockI3D (void)
    private delegate wglDisableGenlockI3D (IntPtr IntPtr)
    private delegate wglDXCloseDeviceNV (HANDLE hDevice)
    private delegate wglDXLockObjectsNV (HANDLE hDevice, GLint count, HANDLE *hObjects)
    private delegate wglDXObjectAccessNV (HANDLE hObject, GLenum access)
    private delegate wglDXOpenDeviceNV (void *dxDevice)
    private delegate wglDXRegisterObjectNV (HANDLE hDevice, void *dxObject, GLuint name, GLenum type, GLenum access)
    private delegate wglDXSetResourceShareHandleNV (void *dxObject, HANDLE shareHandle)
    private delegate wglDXUnlockObjectsNV (HANDLE hDevice, GLint count, HANDLE *hObjects)
    private delegate wglDXUnregisterObjectNV (HANDLE hDevice, HANDLE hObject)
    private delegate wglEnableFrameLockI3D (void)
    private delegate wglEnableGenlockI3D (IntPtr IntPtr)
    private delegate wglEndFrameTrackingI3D (void)
    private delegate wglEnumerateVideoCaptureDevicesNV (IntPtr IntPtr, IntPtr *phDeviceList)
    private delegate wglEnumerateVideoDevicesNV (IntPtr IntPtr, IntPtr *phDeviceList)
    private delegate wglEnumGpuDevicesNV (IntPtr hGpu, UINT iDeviceIndex, PGPU_DEVICE lpGpuDevice)
    private delegate wglEnumGpusFromAffinityDCNV (IntPtr hAffinityDC, UINT iGpuIndex, IntPtr *hGpu)
    private delegate wglEnumGpusNV (UINT iGpuIndex, IntPtr *phGpu)
    private delegate wglFreeMemoryNV (void *pointer)
    private delegate wglGenlockSampleRateI3D (IntPtr IntPtr, UINT uRate)
    private delegate wglGenlockSourceDelayI3D (IntPtr IntPtr, UINT uDelay)
    private delegate wglGenlockSourceEdgeI3D (IntPtr IntPtr, UINT uEdge)
    private delegate wglGenlockSourceI3D (IntPtr IntPtr, UINT uSource)
    private delegate wglGetContextGPUIDAMD (IntPtr IntPtr)
    private delegate wglGetCurrentAssociatedContextAMD (void)
    private delegate wglGetCurrentReadDCARB (void)
    private delegate wglGetCurrentReadDCEXT (void)
    private delegate wglGetDigitalVideoParametersI3D (IntPtr IntPtr, int iAttribute, int *piValue)
    private delegate wglGetExtensionsStringARB (IntPtr IntPtr)
    private delegate wglGetExtensionsStringEXT (void)
    private delegate wglGetFrameUsageI3D (float *pUsage)
    private delegate wglGetGammaTableI3D (IntPtr IntPtr, int iEntries, USHORT *puRed, USHORT *puGreen, USHORT *puBlue)
    private delegate wglGetGammaTableParametersI3D (IntPtr IntPtr, int iAttribute, int *piValue)
    private delegate wglGetGenlockSampleRateI3D (IntPtr IntPtr, UINT *uRate)
    private delegate wglGetGenlockSourceDelayI3D (IntPtr IntPtr, UINT *uDelay)
    private delegate wglGetGenlockSourceEdgeI3D (IntPtr IntPtr, UINT *uEdge)
    private delegate wglGetGenlockSourceI3D (IntPtr IntPtr, UINT *uSource)
    private delegate wglGetGPUIDsAMD (UINT maxCount, UINT *ids)
    private delegate wglGetGPUInfoAMD (UINT id, int property, GLenum dataType, UINT size, void *data)
    private delegate wglGetMscRateOML (IntPtr IntPtr, INT32 *numerator, INT32 *denominator)
    private delegate wglGetPbufferDCARB (IntPtr hPbuffer)
    private delegate wglGetPbufferDCEXT (IntPtr hPbuffer)
    private delegate wglGetPixelFormatAttribfvARB (IntPtr IntPtr, int iPixelFormat, int iLayerPlane, UINT nAttributes, const int *piAttributes, FLOAT *pfValues)
    private delegate wglGetPixelFormatAttribfvEXT (IntPtr IntPtr, int iPixelFormat, int iLayerPlane, UINT nAttributes, int *piAttributes, FLOAT *pfValues)
    private delegate wglGetPixelFormatAttribivARB (IntPtr IntPtr, int iPixelFormat, int iLayerPlane, UINT nAttributes, const int *piAttributes, int *piValues)
    private delegate wglGetPixelFormatAttribivEXT (IntPtr IntPtr, int iPixelFormat, int iLayerPlane, UINT nAttributes, int *piAttributes, int *piValues)
    private delegate wglGetSwapIntervalEXT (void)
    private delegate wglGetSyncValuesOML (IntPtr IntPtr, INT64 *ust, INT64 *msc, INT64 *sbc)
    private delegate wglGetVideoDeviceNV (IntPtr IntPtr, int numDevices, IntPtr *hVideoDevice)
    private delegate wglGetVideoInfoNV (IntPtr IntPtrice, unsigned long *pulCounterOutputPbuffer, unsigned long *pulCounterOutputVideo)
    private delegate wglIsEnabledFrameLockI3D (BOOL *pFlag)
    private delegate wglIsEnabledGenlockI3D (IntPtr IntPtr, BOOL *pFlag)
    private delegate wglJoinSwapGroupNV (IntPtr IntPtr, GLuint group)
    private delegate wglLoadDisplayColorTableEXT (const GLushort *table, GLuint length)
    private delegate wglLockVideoCaptureDeviceNV (IntPtr IntPtr, IntPtr hDevice)
    private delegate wglMakeAssociatedContextCurrentAMD (IntPtr IntPtr)
    private delegate wglMakeContextCurrentARB (IntPtr hDrawDC, IntPtr hReadDC, IntPtr IntPtr)
    private delegate wglMakeContextCurrentEXT (IntPtr hDrawDC, IntPtr hReadDC, IntPtr IntPtr)
    private delegate wglQueryCurrentContextNV (int iAttribute, int *piValue)
    private delegate wglQueryFrameCountNV (IntPtr IntPtr, GLuint *count)
    private delegate wglQueryFrameLockMasterI3D (BOOL *pFlag)
    private delegate wglQueryFrameTrackingI3D (DWORD *pFrameCount, DWORD *pMissedFrames, float *pLastMissedUsage)
    private delegate wglQueryGenlockMaxSourceDelayI3D (IntPtr IntPtr, UINT *uMaxLineDelay, UINT *uMaxPixelDelay)
    private delegate wglQueryMaxSwapGroupsNV (IntPtr IntPtr, GLuint *maxGroups, GLuint *maxBarriers)
    private delegate wglQueryPbufferARB (IntPtr hPbuffer, int iAttribute, int *piValue)
    private delegate wglQueryPbufferEXT (IntPtr hPbuffer, int iAttribute, int *piValue)
    private delegate wglQuerySwapGroupNV (IntPtr IntPtr, GLuint *group, GLuint *barrier)
    private delegate wglQueryVideoCaptureDeviceNV (IntPtr IntPtr, IntPtr hDevice, int iAttribute, int *piValue)
    private delegate wglReleaseImageBufferEventsI3D (IntPtr IntPtr, const LPVOID *pAddress, UINT count)
    private delegate wglReleasePbufferDCARB (IntPtr hPbuffer, IntPtr IntPtr)
    private delegate wglReleasePbufferDCEXT (IntPtr hPbuffer, IntPtr IntPtr)
    private delegate wglReleaseTexImageARB (IntPtr hPbuffer, int iBuffer)
    private delegate wglReleaseVideoCaptureDeviceNV (IntPtr IntPtr, IntPtr hDevice)
    private delegate wglReleaseVideoDeviceNV (IntPtr hVideoDevice)
    private delegate wglReleaseVideoImageNV (IntPtr hPbuffer, int iVideoBuffer)
    private delegate wglResetFrameCountNV (IntPtr IntPtr)
    private delegate wglRestoreBufferRegionARB (HANDLE hRegion, int x, int y, int width, int height, int xSrc, int ySrc)
    private delegate wglSaveBufferRegionARB (HANDLE hRegion, int x, int y, int width, int height)
    private delegate wglSendPbufferToVideoNV (IntPtr hPbuffer, int iBufferType, unsigned long *pulCounterPbuffer, BOOL bBlock)
    private delegate wglSetDigitalVideoParametersI3D (IntPtr IntPtr, int iAttribute, const int *piValue)
    private delegate wglSetGammaTableI3D (IntPtr IntPtr, int iEntries, const USHORT *puRed, const USHORT *puGreen, const USHORT *puBlue)
    private delegate wglSetGammaTableParametersI3D (IntPtr IntPtr, int iAttribute, const int *piValue)
    private delegate wglSetPbufferAttribARB (IntPtr hPbuffer, const int *piAttribList)
    private delegate wglSetStereoEmitterState3DL (IntPtr IntPtr, UINT uState)
    private delegate wglSwapBuffersMscOML (IntPtr IntPtr, INT64 target_msc, INT64 divisor, INT64 remainder)
    private delegate wglSwapIntervalEXT (int interval)
    private delegate wglSwapLayerBuffersMscOML (IntPtr IntPtr, int fuPlanes, INT64 target_msc, INT64 divisor, INT64 remainder)
    private delegate wglWaitForMscOML (IntPtr IntPtr, INT64 target_msc, INT64 divisor, INT64 remainder, INT64 *ust, INT64 *msc, INT64 *sbc)
    private delegate wglWaitForSbcOML (IntPtr IntPtr, INT64 target_sbc, INT64 *ust, INT64 *msc, INT64 *sbc)
